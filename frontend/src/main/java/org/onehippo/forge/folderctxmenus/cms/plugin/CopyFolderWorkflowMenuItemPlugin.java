...
public class CopyFolderWorkflowMenuItemPlugin extends AbstractFolderActionWorkflowMenuItemPlugin {
    ...
    public CopyFolderWorkflowMenuItemPlugin(IPluginContext context, IPluginConfig config) {
        ...
        // Override parent behavior: only add menu item if user has copy privilege
        if (userHasCopyFolderPrivilege()) {
            removeAll(); // Remove any components added by parent constructor
            add(createCopyMenuItemWorkflow());
        } else {
    ...
    /**
     * Creates the copy workflow menu item component.
     */
    private StdWorkflow<FolderWorkflow> createCopyMenuItemWorkflow() {
